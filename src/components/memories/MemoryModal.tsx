...
function MemoryModalContent({
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setDeleteError(
        err instanceof Error ? err.message : "Could not delete this memory.",
      );
      setIsDeleting(false);
      return;
    }
          ...
          <div
            className={`mt-6 ${
              confirmDelete || deleteError
                ? "rounded-xl border border-[var(--border)] bg-[var(--bg)]/50 p-3"
                : ""
            }`}
          >
            {deleteError && (
              <p className="mb-3 rounded-md bg-red-500/10 px-3 py-2 text-xs font-medium text-red-500">
                {deleteError}
              </p>
            )}

            {confirmDelete && (
              <p className="mb-3 text-xs leading-5 text-[var(--body)]/60">
                This will permanently delete this memory from the database.
              </p>
            )}
            ...
            <div className="flex justify-end gap-2">
              {confirmDelete && (
                <button
                  ...
                  disabled={isDeleting}
                  className="rounded-lg px-3 py-1.5 text-xs font-medium text-[var(--body)]/60 transition hover:bg-[var(--surface2)] hover:text-[var(--body)] disabled:opacity-50"
                >
                  Cancel
                </button>
              )}
              ...
              <button
                type="button"
                aria-label={
                  confirmDelete ? "Delete memory permanently" : "Delete memory"
                }
                onClick={handleDelete}
                disabled={isDeleting}
                className={`flex items-center justify-center gap-1.5 rounded-lg text-xs font-semibold transition ${
                  confirmDelete
                    ? "bg-red-500 px-3 py-1.5 text-white hover:bg-red-600"
                    : "p-1 text-[var(--body)]/30 hover:text-red-500"
                } disabled:opacity-50`}
              >
                {isDeleting ? (
